...
def get_dominant_color_and_brightness(img: Image.Image):
    ...
    pixels = [
        p for p in small.getdata() if (len(p) == 3) or (len(p) == 4 and p[3] > 128)
    ]
    if not pixels:
        return (255, 0, 0), "dark"
...
async def get_thumb(videoid: str) -> str:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if not ok:
            ...
            async with aiofiles.open(thumb_path, "wb"):
    ...
    shadow_pad = 24
    shadow = Image.new(
        "RGBA", (thumb_w + shadow_pad, thumb_h + shadow_pad), (0, 0, 0, 0)
    )
    ...
    sdraw.rounded_rectangle(
        (
            shadow_pad // 2,
            shadow_pad // 2,
            thumb_w + shadow_pad // 2,
            thumb_h + shadow_pad // 2,
        ),
        radius=34,
        fill=(0, 0, 0, 170),
    )
    ...
    title_y = thumb_y + 5
    wrapped_title = wrap_text_multilingual(
        title, title_font, text_max_w, max_lines=2, draw=draw
    )
    title_heights = []
    for line in wrapped_title:
        ...
        draw_text_with_shadow(
            draw, (text_x, title_y + sum(title_heights)), line, title_font, text_color
        )
    ...
    gdraw.line(
        [(bar_start, bar_y), (bar_start + prog_len, bar_y)], fill=dom_color, width=32
    )
    glow = glow.filter(ImageFilter.GaussianBlur(20))
    bg = Image.alpha_composite(bg, glow)
    draw = ImageDraw.Draw(bg)
    draw.line(
        [(bar_start, bar_y), (bar_start + prog_len, bar_y)], fill=dom_color, width=9
    )
    draw.line(
        [(bar_start + prog_len, bar_y), (bar_start + total_len, bar_y)],
        fill="#444444",
        width=7,
    )
    draw.ellipse(
        [
            (bar_start + prog_len - 10, bar_y - 10),
            (bar_start + prog_len + 10, bar_y + 10),
        ],
        fill=dom_color,
    )
    ...
    draw_text_with_shadow(
        draw, (bar_start, bar_y + 18), current_time_text, time_font, meta_color
    )
    ...
    draw_text_with_shadow(
        draw,
        (bar_start + total_len - end_width, bar_y + 18),
        end_text,
        time_font,
        end_fill,
    )
